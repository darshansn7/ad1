...
resource "null_resource" "install_package" {
  ...
  provisioner "local-exec" {
    command = "ansible-playbook -i ${path.module}/inventory/hosts ${path.module}/playbooks/main.yml"
  }
  depends_on = [null_resource.azcli, null_resource.packages_download, local_file.update_vars_yaml, local_file.playbooks]
}
